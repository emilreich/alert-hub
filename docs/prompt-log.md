...
Verbatim record of the prompts that drove this build, in order. This is the
candidate's side of the conversation with the AI assistant (Claude Code)
that produced this repo. Assistant responses are summarized rather than
reproduced in full where they were long; anywhere an assistant output was
rejected or corrected, that is called out explicitly and cross-referenced to
`decision-log.md`.

Tool: Claude Code (Sonnet 5), running in-editor against the local
filesystem, with access to shell, file read/write, and web search.

---

### Prompt 1 — the brief + initial ask

> As part of a job interview process I've received the following task.
...
>
> Can you help me analyze the scope of this task and propose whats the best
> way to achieve this given that I am most capable working with dotnet and
> angular

**Assistant response (summarized):** identified four load-bearing
ambiguities in the brief (meaning of "important"/event source, the channel
extensibility requirement, purpose of the admin view, and the subscription
model), recommended ASP.NET Core + Angular + SQLite/EF Core, proposed a
10-step order of work, and asked two clarifying questions via a structured
question tool rather than assuming: (1) new repo vs. folder in the existing
`zenty` repo, (2) per-user filtered subscriptions vs. global alert types.

### Prompt 2 — clarifying answers

> Should this interview solution live in a new, separate GitHub repo, or
> somewhere inside the current zenty repo (e.g. a new folder/branch)? → **New
> separate repo (Recommended)**
>
> How ambitious should the subscription/rule model be, given the 24h budget?
> → **Per-user filtered subscriptions (Recommended)**

Recorded as decisions D1 and D2 in `decision-log.md`.

### Prompt 3 — logistics

> I don't have a github repo I'll let you create the project and I'll add it
> to github once it is created. I agree with your second idea about
> documenting the progress from the get go

Confirms: (a) scaffold locally, candidate pushes to GitHub later, (b)
document intermediate/rejected AI outputs in the decision log as they occur,
not just final prompts. This entry in the prompt log itself is the first
instance of following through on that agreement.

---

<!-- Append further prompts below in the same format as the build proceeds. -->
